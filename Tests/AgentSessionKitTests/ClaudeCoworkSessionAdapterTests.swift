...
final class ClaudeCoworkSessionAdapterTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        home = FileManager.default.temporaryDirectory
            .appendingPathComponent("ASKCoworkSessionTests-\(UUID().uuidString)", isDirectory: true)
    ...
    private func projectDirectory(workspace: String? = nil) -> URL {
        ClaudeCoworkPaths.root(homeDirectory: home.path)
    ...
    func testFilesOutsideTheProjectsDirectoryAreIgnored() throws {
        try writeSession()
        let stray = ClaudeCoworkPaths.root(homeDirectory: home.path)
            .appendingPathComponent(space, isDirectory: true)
    ...
    func testTheSweepMatchesTheCostScannersOwn() throws {
        try writeSession()
        let scanned = ClaudeCoworkPaths.collectJSONL(
            under: ClaudeCoworkPaths.root(homeDirectory: home.path)
        )
        XCTAssertEqual(
    ...
    func testMetadataUsesClaudeCodesParserWithTheCoworkHarness() throws {
        ...
        XCTAssertEqual(summary.createdAt,
                       ISO8601DateFormatter.kitTest.date(from: "2026-05-02T09:00:00.000Z"))
        XCTAssertEqual(summary.lastActiveAt,
                       ISO8601DateFormatter.kitTest.date(from: "2026-05-02T09:00:12.000Z"))
    ...
    func testDeletionIsRefusedBecauseTheFilesAreInsideClaudeAppsContainer() throws {
        ...
        XCTAssertEqual(
            ...
            "Cowork sessions live inside Claude.app's container and are never removed."
        )
