...
public enum MCPSocketError: Error, Sendable, Equatable {
    ...
    /// copy of the host app (a source build alongside the installed one).
    ...
    public var message: String {
        ...
        case let .socketOwnedByAnotherInstance(path):
            return "Another instance already owns the MCP socket at \(path). "
                + "Quit the other copy — agents are still being served by it."
        }
    }
}

/// Whatever answers a framed JSON-RPC line.
///
/// The transport knows nothing about MCP methods, tools, or resources: it
/// reframes bytes into lines and hands each one over. `nil` means "no reply"
/// — a JSON-RPC notification — and the connection stays open.
public protocol MCPLineHandler: Sendable {
    func handle(line: Data) async -> Data?
}

/// What the listener is doing, for a settings surface.
...
/// and the caller is expected to place it inside a 0700 directory, so the
/// filesystem is the whole authentication story: a process that can open it
/// is already running as the user, and a token file would only add a secret
/// to leak. Nothing is ever bound to a network interface.
///
...
public final class MCPSocketServer: @unchecked Sendable {
    ...
    /// Called once, immediately before the first filesystem touch, so the
    /// host can create and lock down the directory the socket goes in. The
    ...
    ///   - socketPath: where to bind. No default: the package does not pick
    ///     a location under the user's home on its host's behalf.
    ///   - ensureDirectory: run before the first filesystem touch of
    ///     `start()`. Create the socket's parent directory here (0700) and
    ///     throw to abort the start.
    public init(
    ...
    public func start() throws {
        ...
        // The host's hook, not our own mkdir: only the caller knows which
        // directory is *its* to create and force to 0700, and chmod-ing a
        // path we were merely handed is not this server's business.
        try ensureDirectory()
        ...
        if FileManager.default.fileExists(atPath: socketPath) {
            guard !Self.isAnyoneListening(atPath: socketPath) else {
                ...
                stateLock.unlock()
                KitLog.warn(
                    "MCP server did not start: \(KitLog.sanitize(socketPath)) is served by another instance."
                )
        ...
        source.resume()
        KitLog.info("MCP server listening on \(KitLog.sanitize(socketPath))")
    }
