...
/// underneath us. `deletionPlan` fails closed (AGENTS.md § 6), there is no
/// resume command, and the entries carry no model — the run happened
/// server-side, so a host pricing this store would have nothing to price.
public struct GrokBotSessionAdapter: SessionProviderAdapter {
    ...
    /// Deliberately not `FileManager.applicationSupportDirectory`: every
