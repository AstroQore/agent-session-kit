...
/// A local session store this package can list, read, resume, and — for the
/// four providers that own their files outright — delete.
///
...
///
/// Eight providers are listed and readable but never deletable, because
/// another running app — or, for Grok Bot and Muse, a server — owns the store; see
/// `supportsDeletion`.
public enum SessionProvider: String, Codable, Sendable, CaseIterable, Hashable {
    ...
    public var displayName: String {
        ...
        case .museAgent: return HarnessCatalog.museAgent
        }
    }
    ...
    public var defaultHarness: Harness {
        ...
        case .museAgent: return .museAgent
        }
    }

    /// Whether this package will ever remove the provider's session files.
    ///
    ...
    /// Mistral Vibe's session directories, which the CLI indexes, leases,
    /// and nests sub-agent sessions inside, and Muse's conversation cache,
    /// which the running app rewrites and whose conversations live on Meta's
    /// servers.
    ...
    public var supportsDeletion: Bool {
        ...
        case .claudeCowork, .cursor, .antigravity, .grokBot, .muse, .devin, .mistralVibe, .museAgent:
            return false
        }
    }
}
...
public struct SessionSummary: Identifiable, Hashable, Codable, Sendable {
    ...
    /// usage axis, which the provider alone cannot answer for Codex
    ...
    /// Canonicalize it for *display* if you like; never canonicalize what is
    /// stored, because pricing tables are matched on the upstream label the
...
public enum SessionDeleteError: Error, Hashable, Codable, Sendable {
    ...
    public var message: String {
        ...
        case let .providerIsReadOnly(provider):
            ...
            case .claudeCowork:
                return "Cowork sessions live inside Claude.app's container and are never removed."
            case .cursor:
            ...
            case .museAgent:
                return "Muse sessions are the app's own cloud cache; manage them in Muse."
